...
def build_data(
    ...
    return {
        ...
        "method": {
            ...
            "moduleEffects": "enabled by default. Supported drive-performance, auxiliary-power, and waste-heat effects are applied to modified chart values when a compatible ship-design module selection is active; unsupported module rules are preserved as visible diagnostics.",
...
CLIENT_ASSET_SENTINEL = ".ti-engine-chart-generated"
...
def copy_client_modules(output_html: Path) -> Path:
    ...
    managed_docs_target = (ROOT / "docs" / CLIENT_ASSET_SUBDIR).resolve()
    ...
    if target_dir.exists():
        if not target_dir.is_dir():
            raise SystemExit(f"Refusing to replace non-directory client asset path: {resolved_target}")
        sentinel = target_dir / CLIENT_ASSET_SENTINEL
        if resolved_target != managed_docs_target and not sentinel.is_file():
            raise SystemExit(
                "Refusing to replace existing client asset directory without "
                f"a generated-assets marker: {resolved_target}"
            )
        shutil.rmtree(target_dir)
    shutil.copytree(DRIVE_COMPARISON_CLIENT_DIR, target_dir)
    (target_dir / CLIENT_ASSET_SENTINEL).write_text(
        "Generated by tools/build_drive_comparison.py.\n",
        encoding="utf-8",
    )
    return target_dir
